...
ridership_routesum <- RS_RidershipImport(rs_filepaths[which(rs_filepaths %ilike% whichmonth)])


#now we'll grab the transaction data
td_filepaths <- dir('data//raw//', full.names = T, pattern = "TRANSACTION DETAIL")
td_filepaths[which(td_filepaths %ilike% whichmonth)] #there it is
...
TD_Mar <- TD_TransactionImport(td_filepaths[5])
#let's do our transit day stuff now
#add a dt with TD data
TD_Mardt <- data.table(TD_Mar)

#grab only the used transactions
route_90_TD_Used <- TD_Mardt[Route %like% "^90"][Description %in% rules$Code | 
                                         Type %like% "118"]
...
route_90_TD_Used <-route_90_TD_Used[, Transit_Day := ifelse(DateTest == 1,
                                         ...
                                         as_date(Date))][,Transit_Day := as_date("1970-01-01")+days(Transit_Day)][
                                           Transit_Day >= "2020-03-01" & Transit_Day < "2020-04-01"
                                           ]
...
#and let's do it
farediff <- GenerateFareDiff(TD_Mar, rules, ridership_routesum)
farediff %>% view()
#grab our VMH data
...
VMH_Raw <- tbl(con, dbplyr::in_schema("avl", "Vehicle_Message_History")) %>%
  filter(Time > "2020-02-29 03:00:00", Time < "2020-04-01 03:00:00",
...
Local_Ridership_90_month_count
...
rolljointable[GPSStatus != 2 | is.na(GPSStatus)] %>% view()
...
#and by date
summary_by_date <- rolljointable[Local_Route != "90", .(total_ridership = .N), by = .(Transit_Day, Local_Route)]

summary_by_date_not_split <- rolljointable[Local_Route != "90",.(total_ridership = .N), by =Transit_Day]

monthly_summary <- rolljointable[Local_Route != "90", .(total_ridership = .N), by = .(month.name[month(Transit_Day)],Local_Route)]



#order aaaaaand write!
fwrite(summary_by_type[order(-Service_Type,Local_Route)], file = "data//processed//March_Local_Ridership_by_typeV3.csv")
fwrite(summary_by_date[order(Transit_Day, Local_Route)], file = "data//processed//March_Local_Ridership_by_dateV3.csv")
fwrite(summary_by_date_not_split[order(Transit_Day)],file = "data//processed//March_Local_Ridership_by_date_not_splitV3.csv")
fwrite(monthly_summary[order(Local_Route)],file = "data//processed//March_SummaryV3.csv")

cbind(fread("data//processed//March_Local_Ridership_by_date_not_splitV3.csv"),
fread("data//processed//March_Local_Ridership_by_date_not_splitV2.csv"),
fread("data//processed//March_Local_Ridership_by_date_not_split.csv"))
